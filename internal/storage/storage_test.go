...
func TestSnapshots(t *testing.T) {
	for _, backend := range []string{storage.SQLite, storage.PostgreSQL} {
		t.Run(backend, func(t *testing.T) {
			...
			t.Run("missing and saved zero differ", func(t *testing.T) {
			...
			t.Run("exact timestamps and independent windows", func(t *testing.T) {
				cases := []struct {
					id    accesskey.ID
					state budget.State
				}{
			...
			t.Run("repeated replacement", func(t *testing.T) {
				config, _ := database(t, backend)
				store := openStore(t, config)
			...
			t.Run("invalid snapshots preserve previous state", func(t *testing.T) {
				config, _ := database(t, backend)
				store := openStore(t, config)
			...
			t.Run("database failure leaves both windows unchanged", func(t *testing.T) {
			...
			t.Run("corrupt read is an error", func(t *testing.T) {
				config, raw := database(t, backend)
				store := openStore(t, config)
			...
			t.Run("keys remain distinct", func(t *testing.T) {
				config, _ := database(t, backend)
				store := openStore(t, config)
			...
			t.Run("cancellation preserves state", func(t *testing.T) {
				config, _ := database(t, backend)
				store := openStore(t, config)
			...
			t.Run("reopen reruns migrations without changing snapshots", func(t *testing.T) {
				config, _ := database(t, backend)
				store := openStore(t, config)
