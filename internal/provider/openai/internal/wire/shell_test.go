...
func TestEncodeShellTools(t *testing.T) {
	for _, tc := range []struct {
		...
		{"omitted", generation.OpenAIShellTool{}, generation.OpenAIShellChoice{}, `{"type":"shell"}`},
		{"null", generation.OpenAIShellTool{Environment: generation.Null[generation.ShellEnvironment](), AllowedCallers: generation.Null[[]string]()}, generation.OpenAIShellChoice{}, `{"type":"shell","environment":null,"allowed_callers":null}`},
		{"local skills", generation.OpenAIShellTool{Environment: generation.Some[generation.ShellEnvironment](generation.ShellLocalEnvironment{Skills: []generation.ShellLocalSkill{{Name: "review", Description: "Read source", Path: "../skills/review"}}}), AllowedCallers: generation.Some([]string{"direct", "programmatic"})}, generation.OpenAIShellChoice{}, `{"type":"shell","allowed_callers":["direct","programmatic"],"environment":{"type":"local","skills":[{"name":"review","description":"Read source","path":"../skills/review"}]}}`},
		{"reference", generation.OpenAIShellTool{Environment: generation.Some[generation.ShellEnvironment](generation.ShellContainerReference{ContainerID: "cntr_1"})}, generation.OpenAIShellChoice{}, `{"type":"shell","environment":{"type":"container_reference","container_id":"cntr_1"}}`},
		{"auto", generation.OpenAIShellTool{Environment: generation.Some[generation.ShellEnvironment](generation.ShellAutoContainer{FileIDs: []string{"file_1"}, MemoryLimit: generation.Null[string](), NetworkPolicy: generation.InterpreterNetworkAllowlist{Domains: []string{"example.org"}, Secrets: []generation.InterpreterDomainSecret{{Domain: "example.org", Name: "TOKEN", Value: "secret"}}}, Skills: []generation.ShellSkill{generation.ShellSkillReference{SkillID: "skill_1", Version: generation.Some("latest")}, generation.ShellInlineSkill{Name: "test", Description: "test skill", Data: "UEs="}}})}, generation.OpenAIShellChoice{}, `{"type":"shell","environment":{"type":"container_auto","file_ids":["file_1"],"memory_limit":null,"network_policy":{"type":"allowlist","allowed_domains":["example.org"],"domain_secrets":[{"domain":"example.org","name":"TOKEN","value":"secret"}]},"skills":[{"type":"skill_reference","skill_id":"skill_1","version":"latest"},{"type":"inline","name":"test","description":"test skill","source":{"type":"base64","media_type":"application/zip","data":"UEs="}}]}}`},
		{"legacy", generation.OpenAILocalShellTool{}, generation.OpenAILocalShellChoice{}, `{"type":"local_shell"}`},
	} {
		t.Run(tc.name, func(t *testing.T) {
			...
			choice := `{"type":"shell"}`
			if tc.name == "legacy" {
				choice = `{"type":"local_shell"}`
			}
			assertJSON(t, got.Choice, choice)
		})
	}
}
...
func TestEncodeShellInputPresence(t *testing.T) {
	request := requestWith(
		generation.OpenAIShellCall{CallID: "call_1", Action: generation.ShellAction{Commands: []string{"echo x", "echo y"}, TimeoutMs: generation.Null[int64](), MaxOutputLength: generation.Some(int64(0))}, ID: generation.Null[string](), Status: generation.Null[string](), Environment: generation.Some[generation.ShellEnvironment](generation.ShellLocalEnvironment{Skills: []generation.ShellLocalSkill{}}), Caller: generation.Null[generation.OpenAIToolCaller]()},
		generation.OpenAIShellResult{CallID: "call_1", MaxOutputLength: generation.Null[int64](), Output: []generation.ShellOutput{{Stdout: "", Stderr: "", Outcome: generation.ShellExit{ExitCode: 0}}, {Stdout: "partial", Stderr: "timed out", Outcome: generation.ShellTimeout{}}}},
		generation.OpenAILocalShellCall{ID: "ls_1", CallID: "local_call", Status: "incomplete", Action: generation.LocalShellAction{Command: []string{"bash", "-lc", "echo x"}, Env: map[string]string{"X": ""}, TimeoutMs: generation.Null[int64](), User: generation.Some(""), WorkingDirectory: generation.Null[string]()}},
...
func TestDecodeShellResponseAndReplay(t *testing.T) {
	...
	call := result.Response.Output[0].(generation.OpenAIShellCall)
	...
	got := result.Response.Output[1].(generation.OpenAIShellResult)
...
func TestDecodeShellClientHandoffs(t *testing.T) {
	for _, output := range []string{
		`[{"type":"shell_call","id":"sh_1","call_id":"call_1","status":"in_progress","action":{"commands":[],"timeout_ms":null,"max_output_length":null},"environment":null}]`,
		`[{"type":"shell_call","id":"sh_1","call_id":"call_1","status":"in_progress","action":{"commands":[],"timeout_ms":null,"max_output_length":null},"environment":{"type":"local"}}]`,
		`[{"type":"local_shell_call","id":"ls_1","call_id":"call_1","status":"in_progress","action":{"type":"exec","command":[],"env":{},"timeout_ms":null}},{"type":"local_shell_call_output","id":"call_1","output":"not JSON","status":null}]`,
	} {
		result, err := shellResponse(output)
		if err != nil {
			t.Fatal(err)
...
func TestRejectMalformedShellResponses(t *testing.T) {
	...
	for _, tc := range []struct{ name, raw string }{
		{"missing id", strings.Replace(call, `"id":"sh_1",`, "", 1)},
		{"null id", strings.Replace(call, `"id":"sh_1"`, `"id":null`, 1)},
		{"missing environment", strings.Replace(call, `,"environment":null`, "", 1)},
		{"missing limit", strings.Replace(call, `"timeout_ms":null,`, "", 1)},
		{"null commands", strings.Replace(call, `["x"]`, `null`, 1)},
		{"null command entry", strings.Replace(call, `["x"]`, `[null]`, 1)},
		{"status", strings.Replace(call, `"completed"`, `"failed"`, 1)},
		{"auto call", strings.Replace(call, `"environment":null`, `"environment":{"type":"container_auto"}`, 1)},
		{"missing output", strings.Replace(output, `"stdout":"",`, "", 1)},
		{"null stdout", strings.Replace(output, `"stdout":""`, `"stdout":null`, 1)},
		{"missing exit", strings.Replace(output, `,"exit_code":0`, "", 1)},
		{"null exit", strings.Replace(output, `"exit_code":0`, `"exit_code":null`, 1)},
		{"null chunks", strings.Replace(output, `[{"stdout":"","stderr":"","outcome":{"type":"exit","exit_code":0}}]`, `null`, 1)},
		{"null output status", strings.Replace(output, `"status":"completed"`, `"status":null`, 1)},
		{"local no env", strings.Replace(local, `,"env":{}`, "", 1)},
		{"local null env value", strings.Replace(local, `"env":{}`, `"env":{"X":null}`, 1)},
		{"local null command", strings.Replace(local, `["x"]`, `[null]`, 1)},
		{"local wrong action", strings.Replace(local, `"exec"`, `"shell"`, 1)},
		{"local missing output", `{"type":"local_shell_call_output","id":"call_1"}`},
	} {
		t.Run(tc.name, func(t *testing.T) {
...
func TestRejectInvalidShellInput(t *testing.T) {
	for _, tc := range []struct {
		...
		{name: "missing local skill path", tool: generation.OpenAIShellTool{Environment: generation.Some[generation.ShellEnvironment](generation.ShellLocalEnvironment{Skills: []generation.ShellLocalSkill{{Name: "test"}}})}},
		{name: "bad inline data", tool: generation.OpenAIShellTool{Environment: generation.Some[generation.ShellEnvironment](generation.ShellAutoContainer{Skills: []generation.ShellSkill{generation.ShellInlineSkill{Name: "test", Data: "private-invalid"}}})}},
		{name: "invalid skill version", tool: generation.OpenAIShellTool{Environment: generation.Some[generation.ShellEnvironment](generation.ShellAutoContainer{Skills: []generation.ShellSkill{generation.ShellSkillReference{SkillID: "skill_1", Version: generation.Some("0")}}})}},
	} {
		t.Run(tc.name, func(t *testing.T) {
			request := requestWith()
			if tc.item != nil {
...
func TestDecodeShellOutputs(t *testing.T) {
	...
	for _, raw := range []string{`null`, `{}`, `[{}]`, `[{"stdout":"","stderr":"","outcome":{"type":"timeout","exit_code":1}}]`, `[{"stdout":"","stderr":"","outcome":{"type":"exit","exit_code":0.5}}]`, `[{"stdout":"","stderr":"","outcome":{"type":"exit","exit_code":9223372036854775808}}]`} {
		if _, err := wire.DecodeShellOutputs(json.RawMessage(raw)); err == nil {
			t.Fatalf("accepted malformed outputs: %s", raw)
		}
	}
}
