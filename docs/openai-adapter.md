...
| Files | Upload, retrieve metadata or content, delete |
| Containers | Create, retrieve, delete; upload or attach files, list, retrieve, delete and read file content |
| Vector stores | Create, retrieve, delete; attach, list, retrieve and detach files |
...
List methods return one page and its cursors. Uploads read from caller-supplied
readers; the adapter does not open local files. Resource status is returned as data,
...
File-content methods transfer an `io.ReadCloser` to the caller. Close it after use;
the operation's context controls later reads. Uploads take ownership of an
`io.ReadCloser` and close it on every return path, including validation failure.
Its `Close` must unblock a concurrent `Read`, as required by `net/http`.
...
transport may recover a stale connection for an idempotent read. Generation and
upload bodies cannot be replayed by that transport.
